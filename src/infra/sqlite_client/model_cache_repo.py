...
class ModelCacheRepository:
    ...
    async def get_cached_models(self) -> List[ModelEntity]:
        ...
        """
        try:
            cached_models = (AvailableModelCache
                           .select()
                           .where(AvailableModelCache.is_active == True)
                           .order_by(AvailableModelCache.name))
    ...
    async def is_cache_valid(self) -> bool:
        ...
        try:
            # 最新の更新時刻を取得
            latest_update = (AvailableModelCache
                           .select(AvailableModelCache.last_updated)
                           .where(AvailableModelCache.is_active == True)
                           .order_by(AvailableModelCache.last_updated.desc())
                           .first())
    ...
    async def update_cache(self, models: List[ModelEntity]) -> None:
        ...
        try:
            ...
            if cache_data:
                # バッチ挿入
                AvailableModelCache.insert_many(cache_data).execute()
    ...
    async def model_exists_in_cache(self, model_id: str) -> bool:
        ...
        """
        try:
            exists = (AvailableModelCache
                     .select()
                     .where(
                         (AvailableModelCache.id == model_id) &
                         (AvailableModelCache.is_active == True)
                     )
                     .exists())
            return exists
        except Exception:
